...
if [ -x /usr/bin/curl ]; then
    exec_cmd_nobail "curl -skLf -o /dev/null 'https://deb.nodesource.com/node_4.x/dists/${DISTRO}/Release'"
    RC=$?
else
...
if [ -x /usr/bin/curl ]; then
    exec_cmd 'curl -sk https://deb.nodesource.com/gpgkey/nodesource.gpg.key | apt-key add -'
else
